...
userController.checkUserExist = async (req, res) => {
    try {
      const payload = req.body;
      let isEmailExist = await findOneUser({ email: payload.email, isDeleted: { $ne: true } });
      let isMobileExist = await findOneUser({ mobile: payload.mobile, isDeleted: { $ne: true } });
      
      res.status(200).json({ email: !!isEmailExist, mobile: !!isMobileExist });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: INTERNAL_SERVER_ERROR });
    }
